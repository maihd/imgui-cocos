...
public:
    ...
    static void setGLViewImpl(ImGuiGLView* view)
    {
        _view = view;
    }
...
private:
    static ImGuiGLView* _view;
};

ImGuiGLView* ImGuiEventHandler::_view = nullptr;

const std::string ImGuiGLView::EVENT_WINDOW_RESIZED = "glview_window_resized";
const std::string ImGuiGLView::EVENT_WINDOW_FOCUSED = "glview_window_focused";
const std::string ImGuiGLView::EVENT_WINDOW_UNFOCUSED = "glview_window_unfocused";
...
//////////////////////////////////////////////////////////////////////////
// implement ImGuiGLView
//////////////////////////////////////////////////////////////////////////


ImGuiGLView::ImGuiGLView(bool initglfw)
: _captured(false)
, _supportTouch(false)
...
, _mouseY(0.0f)
{
    ...
    ImGuiEventHandler::setGLViewImpl(this);
    if (initglfw)
    {
        glfwSetErrorCallback(ImGuiEventHandler::onGLFWError);
        glfwInit();
    }
}

ImGuiGLView::~ImGuiGLView()
{
...
ImGuiGLView* ImGuiGLView::create(const std::string& viewName)
{
    return ImGuiGLView::create(viewName, false);
}

ImGuiGLView* ImGuiGLView::create(const std::string& viewName, bool resizable)
{
    auto ret = new (std::nothrow) ImGuiGLView;
...
ImGuiGLView* ImGuiGLView::createWithRect(const std::string& viewName, Rect rect, float frameZoomFactor, bool resizable)
{
    auto ret = new (std::nothrow) ImGuiGLView;
...
ImGuiGLView* ImGuiGLView::createWithFullScreen(const std::string& viewName)
{
    auto ret = new (std::nothrow) ImGuiGLView();
...
ImGuiGLView* ImGuiGLView::createWithFullScreen(const std::string& viewName, const GLFWvidmode &videoMode, GLFWmonitor *monitor)
{
    auto ret = new (std::nothrow) ImGuiGLView();
...
bool ImGuiGLView::initWithRect(const std::string& viewName, Rect rect, float frameZoomFactor, bool resizable)
{
    ...
    */
    ...
    glfwSetMouseButtonCallback(_mainWindow, ImGuiEventHandler::onGLFWMouseCallBack);
    glfwSetCursorPosCallback(_mainWindow, ImGuiEventHandler::onGLFWMouseMoveCallBack);
    glfwSetScrollCallback(_mainWindow, ImGuiEventHandler::onGLFWMouseScrollCallback);
    glfwSetCharCallback(_mainWindow, ImGuiEventHandler::onGLFWCharCallback);
    glfwSetKeyCallback(_mainWindow, ImGuiEventHandler::onGLFWKeyCallback);
    glfwSetWindowPosCallback(_mainWindow, ImGuiEventHandler::onGLFWWindowPosCallback);
    glfwSetFramebufferSizeCallback(_mainWindow, ImGuiEventHandler::onGLFWframebuffersize);
    glfwSetWindowSizeCallback(_mainWindow, ImGuiEventHandler::onGLFWWindowSizeFunCallback);
    glfwSetWindowIconifyCallback(_mainWindow, ImGuiEventHandler::onGLFWWindowIconifyCallback);
    glfwSetWindowFocusCallback(_mainWindow, ImGuiEventHandler::onGLFWWindowFocusCallback);
...
bool ImGuiGLView::initWithFullScreen(const std::string& viewName)
{
...
bool ImGuiGLView::initWithFullscreen(const std::string &viewname, const GLFWvidmode &videoMode, GLFWmonitor *monitor)
{
...
bool ImGuiGLView::isOpenGLReady()
{
    return nullptr != _mainWindow;
}

void ImGuiGLView::end()
{
    if(_mainWindow)
    {
    ...
    // Release self. Otherwise, ImGuiGLView could not be freed.
    release();
}

void ImGuiGLView::swapBuffers()
{
    if(_mainWindow)
        glfwSwapBuffers(_mainWindow);
}

bool ImGuiGLView::windowShouldClose()
{
    if(_mainWindow)
...
void ImGuiGLView::pollEvents()
{
    glfwPollEvents();
}

void ImGuiGLView::enableRetina(bool enabled)
{
...
void ImGuiGLView::setIMEKeyboardState(bool /*bOpen*/)
{

}

void ImGuiGLView::setCursorVisible( bool isVisible )
{
    if( _mainWindow == NULL )
        return;
...
void ImGuiGLView::setFrameZoomFactor(float zoomFactor)
{
...
float ImGuiGLView::getFrameZoomFactor() const
{
    return _frameZoomFactor;
}

bool ImGuiGLView::isFullscreen() const {
    return (_monitor != nullptr);
}

void ImGuiGLView::setFullscreen() {
...
void ImGuiGLView::setFullscreen(int monitorIndex) {
...
void ImGuiGLView::setFullscreen(const GLFWvidmode &videoMode, GLFWmonitor *monitor) {
    _monitor = monitor;
    glfwSetWindowMonitor(_mainWindow, _monitor, 0, 0, videoMode.width, videoMode.height, videoMode.refreshRate);
}

void ImGuiGLView::setWindowed(int width, int height) {
    if (!this->isFullscreen()) {
...
int ImGuiGLView::getMonitorCount() const {
    int count = 0;
    glfwGetMonitors(&count);
    return count;
}

Size ImGuiGLView::getMonitorSize() const {
...
void ImGuiGLView::updateFrameSize()
{
...
void ImGuiGLView::setFrameSize(float width, float height)
{
    GLView::setFrameSize(width, height);
    updateFrameSize();
}

void ImGuiGLView::setViewPortInPoints(float x , float y , float w , float h)
{
...
void ImGuiGLView::setScissorInPoints(float x , float y , float w , float h)
{
...
Rect ImGuiGLView::getScissorRect() const
{
    GLfloat params[4];
...
void ImGuiGLView::onGLFWError(int errorID, const char* errorDesc)
{
    if (_mainWindow)
    {
...
void ImGuiGLView::onGLFWMouseCallBack(GLFWwindow* /*window*/, int button, int action, int /*modify*/)
{
...
void ImGuiGLView::onGLFWMouseMoveCallBack(GLFWwindow* window, double x, double y)
{
    _mouseX = (float)x;
    _mouseY = (float)y;
...
void ImGuiGLView::onGLFWMouseScrollCallback(GLFWwindow* /*window*/, double x, double y)
{
...
void ImGuiGLView::onGLFWKeyCallback(GLFWwindow* /*window*/, int key, int /*scancode*/, int action, int /*mods*/)
{
...
void ImGuiGLView::onGLFWCharCallback(GLFWwindow* /*window*/, unsigned int character)
{
...
void ImGuiGLView::onGLFWWindowPosCallback(GLFWwindow* /*window*/, int /*x*/, int /*y*/)
{
    Director::getInstance()->setViewport();
}

void ImGuiGLView::onGLFWframebuffersize(GLFWwindow* window, int w, int h)
{
...
void ImGuiGLView::onGLFWWindowSizeFunCallback(GLFWwindow* /*window*/, int width, int height)
{
    if (width && height && _resolutionPolicy != ResolutionPolicy::UNKNOWN)
    {
        ...
        Director::getInstance()->setViewport();
        Director::getInstance()->getEventDispatcher()->dispatchCustomEvent(ImGuiGLView::EVENT_WINDOW_RESIZED, nullptr);
    }
}

void ImGuiGLView::onGLFWWindowIconifyCallback(GLFWwindow* /*window*/, int iconified)
{
...
void ImGuiGLView::onGLFWWindowFocusCallback(GLFWwindow* /*window*/, int focused)
{
    if (focused == GL_TRUE)
    {
        Director::getInstance()->getEventDispatcher()->dispatchCustomEvent(ImGuiGLView::EVENT_WINDOW_FOCUSED, nullptr);
    }
    else
    {
        Director::getInstance()->getEventDispatcher()->dispatchCustomEvent(ImGuiGLView::EVENT_WINDOW_UNFOCUSED, nullptr);
    }
}
